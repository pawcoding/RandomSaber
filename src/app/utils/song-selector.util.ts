...
export class SongSelectorUtil {
   ...
   */
  public static getPlayableSongs(packs: Pack[], mode: Mode, difficulty: Difficulty): SongToPlay[] {
    ...
    console.log(`[SongSelector]\tCollected ${songs.length} songs with "${mode}" on "${difficulty}" in ${elapsedTimeInMs.toFixed(4)}ms.`)
   ...
   */
  private static _isPlayable(song: Song, difficultyNumber: number, mode: Mode): boolean {
    if (!song.active)
      return false
